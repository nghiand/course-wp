...
<div class="quiz-buttons">
    <?php if( ! learn_press_user_has_started_quiz() ):?>
        ...
        <button class="button-start-quiz btn" quiz-id="<?php echo get_the_ID() ?>">
            ...
            echo apply_filters( 'learn_press_start_quiz_button_text', __( "Start Quiz", "thim" ) );
            ?>
        </button>
    ...
    <button class="button-finish-quiz btn" quiz-id="<?php echo get_the_ID() ?>" data-area="sidebar">
        <?php
        // allow doing quiz if enrolled
        echo apply_filters( 'learn_press_sidebar_finish_quiz_text', __( "Finish Quiz", "thim" ) );
        ?>
    </button>
    <?php do_action( 'learn_press_after_finish_quiz_button' );?>
    <?php //endif;?>
    <?php if( learn_press_user_can_retake_quiz() ):?>
        <button class="button-retake-quiz btn" data-id="<?php the_ID();?>"><?php _e( 'Retake', 'thim' );?></button>
    <?php endif;?>
</div>
