...
if ( is_page() || is_single() ) {
	...
	if(get_post_type() == "lp_course") {
		if(!empty($theme_options_data['thim_learnpress_cate_sub_title']))
			$subtitle = $theme_options_data['thim_learnpress_cate_sub_title'];
	}
}
	...
	<div class="top_site_main" <?php echo ent2ncr( $c_css ); ?>>
		...
		<div class="page-title-wrapper">
			<div class="banner-wrapper container">
				...
				} elseif ( ( is_category() || is_archive() || is_search() || is_404() ) && ! use_bbpress() ) {
...
//				}
				elseif ( is_page() || is_single() ) {
					...
					echo '<h1>';
					echo ( $front_title != '' ) ? $front_title : __( 'Home', 'thim' );
					echo '</h1>';
					echo ( $subtitle != '' ) ? '<div class="banner-description" ' . $c_css_sub_color . '><p>' . $subtitle . '</p></div>' : '';
				}
				?>
...
$c_css_overlay = ( $cate_top_image_src != '' && $bg_color != '' ) ? '<span class="overlay-top-header" style="background-color:' . $bg_color . '"></span>' : '';
if ( $hide_title == '1' && $theme_options_data['thim_header_position'] == 'header_overlay' && $c_css_1 != '' ) { ?>
...
if ( ( get_post_type() == "product" || get_post_type() == "post" || get_post_type() == "lpr_course" || get_post_type() == "lp_course" ) && ! is_single() ) {
	...
	$list_grid = '<div class="display grid-list-switch ' . $post_type . '-switch" data-cookie="' . $post_type . '-switch">
		<a href="javascript:;" class="list switchToGrid">' . __( 'Grid', 'thim' ) . '<i class="fa fa-th-large"></i></a>
		<a href="javascript:;" class="grid switchToList">' . __( 'List', 'thim' ) . '<i class="fa fa-list-ul"></i></a>
	</div>';
}
?>
...
<?php
if ( $hide_breadcrumbs != '1' || $list_grid ) {
	if ( ! use_bbpress() && ! is_buddypress_use_page() && ( ! is_front_page() || ! is_home() ) ) { ?>
		<div class="breadcrumbs-wrapper<?php if ( $list_grid ) {
			echo ' button-list-gird';
		} ?>">
			<div class="container">
				<?php
				if ( get_post_type() == 'product' ) {
