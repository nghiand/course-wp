...
foreach ( $instructor as $user_id => $course_id ) {
    $user_social = get_the_author_meta( 'lp_info', $course_id->ID );
    $courses     = 0;
    if ( function_exists( 'learn_press_get_own_courses' ) ) {
        $courses = learn_press_get_own_courses( $course_id->ID );
    ...
    echo '<div class="avatar-instructors">' . get_avatar( $course_id->ID, '480' );
    echo '<span class="number-courses">' . __( 'Courses by instructors', 'thim' ) . ' <b>' . $course_id->ID . '</b></span>';
    echo '</div>';
    echo '<h5><a href="' . learn_press_user_profile_link( $course_id->ID ) . '">' . get_the_author_meta( 'display_name', $course_id->ID ) . '</a></h5>';
