...
$heading       = sprintf( __( 'Other results (newest %d items)', 'thim' ), $limit );
...
if ( $history_count > 1 ) {
	...
	<table class="quiz-history">
		...
		<tr>
			...
			<th><?php _e( 'Time', 'thim' ); ?></th>
			<th><?php _e( 'Result', 'thim' ); ?></th>
		</tr>
		</thead>
		<?php foreach ( $history as $item ) {
			...
			<tr>
				...
				<p class="quiz-history-actions">
					<a href="<?php echo add_query_arg( 'history_id', $item->history_id ); ?>"><?php _e( 'View', 'thim' ); ?></a>
					<a href=""><?php _e( 'Use as result', 'thim' ); ?></a>
				</p>
				-->
				</td>
			</tr>
	...
	learn_press_display_message( __( 'No history found!', 'thim' ) );
}