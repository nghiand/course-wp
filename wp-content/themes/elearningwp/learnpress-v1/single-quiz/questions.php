...
$heading      = apply_filters( 'learn_press_list_questions_heading', __( 'List of questions', 'thim' ) );
		...
		<ul class="quiz-questions-list">
			<?php if ( $questions = $quiz->get_questions() ) foreach ( $questions as $question ) { ?>
				<li data-id="<?php echo $question->ID; ?>" <?php learn_press_question_class( $question->ID, array( 'user' => $user, 'quiz' => $quiz ) ); ?>>
					...
					<?php if ( $no_permalink ) { ?>
						<?php printf( '<h5 class="question-title">%s</h5>', get_the_title( $question->ID ) ); ?>
					<?php } else { ?>
						<?php printf( '<h5 class="question-title"><a href="%s">%s</a></h5>', $quiz->get_question_link( $question->ID ), get_the_title( $question->ID ) ); ?>
					<?php } ?>
...
<?php else: ?>
	...
	<?php learn_press_display_message( apply_filters( 'learn_press_quiz_no_questions_notice', __( 'This quiz hasn\'t got any questions', 'thim' ) ) ); ?>
