...
if( !is_user_logged_in() ){
	learn_press_display_message( sprintf( __( 'You are not logged in! Please <a href="%s">login</a> to save the results. The results will be deleted after your session destroyed', 'thim' ), learn_press_get_login_url() ), 'error' );
}
...
<div class="quiz-result">
	<h4 class="result-title"><?php _e( 'Your result', 'thim' ); ?></h4>
	...
	<div class="quiz-result-mark">
		...
		<small><?php _e( 'point', 'thim' ); ?></small>
	</div>
	<div class="quiz-result-summary">
		<div class="quiz-result-field correct">
			<label><?php echo apply_filters( 'learn_press_quiz_result_correct_text', __( 'Correct', 'thim' ) ); ?></label>
			<?php printf( "%d (%0.2f%%)", $history->results['correct'], $history->results['correct_percent'] ); ?>
		</div>
		<div class="quiz-result-field wrong">
			<label><?php echo apply_filters( 'learn_press_quiz_result_wrong_text', __( 'Wrong', 'thim' ) ); ?></label>
			<?php printf( "%d (%0.2f%%)", $history->results['wrong'], $history->results['wrong_percent'] ); ?>
		</div>
		<div class="quiz-result-field empty">
			<label><?php echo apply_filters( 'learn_press_quiz_result_empty_text', __( 'Empty', 'thim' ) ); ?></label>
			<?php printf( "%d (%0.2f%%)", $history->results['empty'], $history->results['empty_percent'] ); ?>
		</div>
		<div class="quiz-result-field time">
			<label><?php echo apply_filters( 'learn_press_quiz_result_time_text', __( 'Time', 'thim' ) ); ?></label>
